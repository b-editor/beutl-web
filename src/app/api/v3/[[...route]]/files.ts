...
async function isAllowed(file: NonNullable<Awaited<ReturnType<typeof findFile>>>, userId: string | null) {
...
const app = new Hono()
  .get("/:id", async (c) => {
    ...
    if (!await isAllowed(file, userId)) {
      return c.json(await apiErrorResponse("doNotHavePermissions"), { status: 403 });
    }
