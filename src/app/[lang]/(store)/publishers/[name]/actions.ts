...
export async function retrievePublishedPackages(userName: string): Promise<{ items: ListedPackage[], displayName: string }> {
  ...
  const items = Promise.all(tmp.map((up) => up.package).map(async (pkg) => {
