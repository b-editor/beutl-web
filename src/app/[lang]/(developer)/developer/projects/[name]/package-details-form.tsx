...
export function PackageDetailsForm({ pkg }: { pkg: Package }) {
  const [tags, manipulateTags] = useOptimistic<string[], { type: "add" | "delete", tag: string } | { type: "reset", tags: string[] }
  >(pkg.tags, (state, req) => {
    if (req.type === "add") {
  ...
  const handleAddTag = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    const res = await updateTag({ packageId: pkg.id, tags: [...tags, newTag] });
    if (!res.success) {
      toast({
        title: "エラー",
  ...
  const handleDeleteTag = useCallback(async (tag: string) => {
    ...
    const res = await updateTag({ packageId: pkg.id, tags: tags.filter(t => t !== tag) });
    if (!res.success) {
      toast({
        title: "エラー",
        description: res.message,
    ...
    <div className="lg:basis-1/3">
      ...
      <div className="flex gap-2 flex-col my-4">
        ...
        <div className="flex gap-1 flex-wrap">
          {pkg.tags.map((tag) => (
            <Badge key={tag} onClick={() => handleDeleteTag(tag)}>{tag}<X className="ml-1 w-4 h-4" /></Badge>
          ))}
          <Popover>
            <PopoverTrigger>
