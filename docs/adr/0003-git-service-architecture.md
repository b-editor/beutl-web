...
1. `revokeGitAccess` — トークンを全て失効させる。**ここが成否の分かれ目**で、
   失敗したら Beutl 側も消さずに中止する
2. Beutl 側のレコードを消す
3. `purgeGitAccount` — トークンをもう一度掃いてから、ユーザーとリポジトリを消す

3 で掃き直すのは、1 と 2 の間に発行された分がありうるため。purge が失敗したときに
生き残るのはまさにそれで、対応表はもう無いので誰にも失効させられない。

purge が失敗しても投げない。アクセスは断ててあるので穴は開かず、Beutl 側の削除は
既に確定していて戻せない。代わりに `git.accountPurgeFailed` を監査に残す。**自動では
再試行されない**ので、これが手で消すための唯一の手がかりになる。
