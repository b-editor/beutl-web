...
したがって退会処理は、`GitAccountDeletion` という「まだ片付いていない」印を軸に進める。

1. `beginGitAccountDeletion` — 身元を確かめ、印を書いてからトークンを失効させる。
   **ここが成否の分かれ目**で、失敗したら Beutl 側も消さずに中止する
2. Beutl 側のレコードを消す
3. `finishGitAccountDeletion` — 掃き直して purge し、成功したら印を消す
...
戻せない。印が残るので `retryPendingGitDeletions` が拾って再試行する。退会処理の
入り口からも呼ぶので、定期実行が無くても退会のたびに前へ進む。
