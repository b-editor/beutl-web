...
トークン管理エンドポイント (`/api/v1/users/{username}/tokens`) は、**書き込みだけ**
対象ユーザー自身の Basic 認証を要求する。Forgejo 16.0.2 での実測:

| 操作 | 管理トークン | 対象ユーザーの Basic 認証 |
| --- | --- | --- |
| `GET` (一覧) | 200 | 200 |
| `POST` (発行) | 401 `auth method not allowed` | 201 |
| `DELETE` (失効) | 401 `auth method not allowed` | 204 |

`Sudo` 代理も同じく受け付けない。一覧だけは管理トークンで取れるが、必要なのは発行と
失効の方なので、この経路は使えない。
...
したがって退会処理は Forgejo を先に消し、失敗したら Beutl 側も消さずに中止する
(`deleteGitAccount`)。逆順にすると、対応表を失った状態で誰のものか分からないユーザーと
生きたトークンだけが残る。既に消えていれば 404 を成功として扱うので、やり直しが通る。
