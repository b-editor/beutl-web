...
export function createStorageOperations(execution?: { waitUntil?(task: Promise<unknown>): void }) {
  ...
  async function deleteStorageFile({
    ...
    try {
      ...
      await deleteAiStorageCleanup({ objectKey: record.objectKey }).catch((cleanupError) =>
        console.error("Failed to acknowledge storage cleanup", record.objectKey, cleanupError),
      );
    } catch (error) {
  ...
  async function calcTotalFileSize({
    userId,
    prisma,
  }: {
    userId: string;
  ...
  async function createStorageFile({
    ...
    try {
      ...
      await deleteAiStorageCleanup({ objectKey }).catch((cleanupError) =>
        console.error("Failed to clear storage write outbox", objectKey, cleanupError),
      );
      return record;
    } catch (error) {
      try {
        ...
        throw new AggregateError(
          [error, cleanupError],
          "Storage record failed and cleanup could not be queued",
        );
      }
      throw error;
    }
  ...
  async function createDedicatedStorageFile({
    file,
    userId,
    quota,
    publish,
    ...
    publish?: (
      ...
      record: { id: string; objectKey: string; size: bigint },
    ) => Promise<void>;
  }) {
  ...
  async function copyAiResultToStorage({
    jobId,
    userId,
    folderId = null,
    saveKey,
    ...
    if (folderId !== null && !(await storageFolderBelongsToUser({ folderId, userId }))) {
    ...
    for (let attempt = 0; attempt < AI_RESULT_COPY_ATTEMPTS; attempt++) {
      const id = `ai-result-copy:${await sha256Hex(`${userId}\n${jobId}\n${saveKey}\n${attempt}`)}`;
  ...
  async function createReservedStorageFile({
    ...
    try {
      ...
      if (
        !(await renewDedicatedStorageReservation({
          id: reservation.reservation.id,
          userId,
          objectKey,
          leaseToken,
      ...
      for (;;) {
        ...
        const now = new Date();
        const nextLeaseUntil = new Date(now.getTime() + DEDICATED_STORAGE_WRITE_LEASE_MILLISECONDS);
        ...
        if (renewalOutcome.kind === "deadline") {
          ...
          const authoritativeLease =
            lateRenewal?.kind === "renewed" && lateRenewal.renewed ? nextLeaseUntil : null;
      ...
      try {
        ...
        throw new AggregateError(
          [error, cleanupError],
          "Dedicated storage write failed and cleanup could not be queued",
        );
      }
      throw error;
    }
  ...
  return {
    ...
    calcTotalFileSize,
    createStorageFile,
    createDedicatedStorageFile,
    copyAiResultToStorage,
  };
}
