...
// 済んだ job の名前が、本文を読ませる口を開けておく期間。
//
// 期限を置くのは、済んだ名前がひとつあれば、それを言い続けるだけで大きな本文を
// 何度でも読ませられるから——契約が終わったあとでも。走っている job には置かな
// い。取りに来る先は結果であって、期限ではない。
//
// 長さは文章の結果を残す期間に合わせてある。これはこの口の期限であって、結果の
// 期限ではない: 画像や動画の結果はもっと長く残り、期間を過ぎても履歴からは取り
// 出せる。ここで閉じるのは「本文を送り直して回収する」道だけで、そちらには期限
// が無い。
